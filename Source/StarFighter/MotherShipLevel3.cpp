...
AMotherShipLevel3::AMotherShipLevel3()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> AssetNodriza3(TEXT("StaticMesh'/Game/Meshes/Nodriza3.Nodriza3'"));
	ShipMeshEnemy->SetStaticMesh(AssetNodriza3.Object);
	...
	EnemyshipName = "MotherShipLevel3";
...
void AMotherShipLevel3::BeginPlay()
{
	Super::BeginPlay();
...
void AMotherShipLevel3::Tick(float DeltaTime)
{
	...
	{
		SpawnTime = 0.f;
		//const FVector MoveDirectionShip = FVector(-1.f, 0.f, 0.f);
		...
		UWorld* const World = GetWorld();
		if (World != nullptr) {
			World->SpawnActor<AEnemyLevel3>(SpawnLocationShip1, Rotation);
			World->SpawnActor<AEnemyLevel3>(SpawnLocationShip2, Rotation);
			World->SpawnActor<AEnemyLevel3>(SpawnLocationShip3, Rotation);
			World->SpawnActor<AEnemyLevel3>(SpawnLocationShip4, Rotation);
		}
	}
	...
	if (Life <= 0.f) {
		...
		// este actor da 300puntos
		LevelPass->setScore(300);
...
void AMotherShipLevel3::WeaponDamage(float Damage)
{
	Life -= Damage;
}

// si esta nave nodiza colisiona con el jugador
void AMotherShipLevel3::NotifyActorBeginOverlap(AActor* OtherActor)
{
	...
	if (Player)
	{
		...
		if (ExplosionSound != nullptr && this->GetActorLocation().X > -850.f)
			UGameplayStatics::PlaySoundAtLocation(this, ExplosionSound, GetActorLocation());
		// si te chocas con este actor te quita 300 puntos
		LevelPass->setScore(-300);
		this->Destroy();
	}
}
